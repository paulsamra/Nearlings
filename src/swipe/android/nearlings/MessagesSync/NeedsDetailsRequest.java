...
//for now we insert with
	...
	if(choice % 2 == 0){
		...
		cv.put(NeedsDetailsDatabaseHelper.COLUMN_LOCATION_GEOPOINT_LATITUDE, latitude);
		cv.put(NeedsDetailsDatabaseHelper.COLUMN_LOCATION_GEOPOINT_LONGITUDE, longitude);
		
		cv.put(NeedsDetailsDatabaseHelper.COLUMN_DESCRIPTION, "Description");
		cv.put(NeedsDetailsDatabaseHelper.COLUMN_AUTHOR_IMAGE_PREVIEW_URL, "http://epicappsolutions.com/wp-content/uploads/2013/10/Android-icon.png");
		cv.put(NeedsDetailsDatabaseHelper.COLUMN_STATUS, "Status");
		cv.put(NeedsDetailsDatabaseHelper.COLUMN_ID_OF_NEED, System.currentTimeMillis());
