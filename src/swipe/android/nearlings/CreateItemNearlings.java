...
public class CreateItemNearlings extends AsyncTask<String, Void, String> {
	...
	public CreateItemNearlings(Activity act,
			AsyncTaskCompleteListener asyncListener) {
		this.activity = act;
	...
	public CreateItemNearlings(AsyncTaskCompleteListener listener, Context ctx,
			boolean showDialog) {
		this.ctx = ctx;
	...
	protected String doInBackground(String... params) {
		...
		try {
			...
			if (createRequest.has("location")) {
				Log.d("Location found", createRequest.toString());
				String location = createRequest.getString("location");
				JSONObject locationInfo = GoogleParser
			...
			Log.d("URL", params[0]);
			s = SocketOperator.getInstance(String.class).httpPostRequest(
					params[0], MapUtils.stringToMap(params[1]),
